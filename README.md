...
The project includes database tests. To run them, navigate to the Database Scheme & Testing folder and run python test_database.py. The tests check that all four database tables — users, courses, students, and enrollments — contain data after the setup scripts have been run. If everything is working, you will see four pass messages in the terminal. For manual testing, register a new student account and confirm it redirects to login. Log in as a student and try enrolling in and dropping a course. Log in as admin and try adding, editing, and deleting a course. Search for a course using a keyword and confirm results are filtered correctly.
